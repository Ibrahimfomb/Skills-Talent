    ...
    >
      <div className="bsm-modal">
        <div className="bsm-header">
          <h2 className="bsm-title">Mise à jour en masse</h2>
          <button className="bsm-close" onClick={onClose}><X size={18} /></button>
        </div>

        <p className="bsm-count">
          <strong>{selectedIds.length}</strong> candidature{selectedIds.length > 1 ? 's' : ''} sélectionnée{selectedIds.length > 1 ? 's' : ''}
        </p>

        {!report ? (
          <>
            <label className="bsm-label">Nouveau statut</label>
            <select
          ...
          <div className="bsm-report">
            ...
            <p className="bsm-report-title">Mise à jour effectuée</p>
            <p className="bsm-report-line">
              <strong>{report.updated}</strong> candidature{report.updated > 1 ? 's' : ''} mise{report.updated > 1 ? 's' : ''} à jour
            </p>
            {report.skipped?.length > 0 && (
              <p className="bsm-report-skip">
                {report.skipped.length} ignorée{report.skipped.length > 1 ? 's' : ''} (accès refusé)
              </p>
            )}
      ...
      <style>{`
        ...
        .bsm-title { font-size: 1.1rem; font-weight: 600; margin: 0; }
        .bsm-close {
          background: none; border: none; cursor: pointer;
          color: var(--color-text-muted, #666); padding: 4px;
        }
        .bsm-count {
          font-size: 0.9rem; color: var(--color-text-muted, #555);
          margin-bottom: 20px;
        }
        .bsm-label {
        ...
        .bsm-select {
          ...
          box-sizing: border-box; background: var(--color-surface, #fff);
          color: var(--color-text, #222); cursor: pointer;
        }
        .bsm-select:focus { border-color: var(--color-primary, #6366f1); outline: none; }
        .bsm-error {
        ...
        .bsm-report-title { font-size: 1rem; font-weight: 600; margin: 0 0 8px; }
        .bsm-report-line { font-size: 0.95rem; color: var(--color-text, #222); margin: 0 0 4px; }
        .bsm-report-skip { font-size: 0.85rem; color: var(--color-text-muted, #888); margin: 0; }
      `}</style>
    </div>
  )
}
